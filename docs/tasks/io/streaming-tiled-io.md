...
`docs/progress/_unassigned.md`). Its cheaper counterpart —
...
1. **Gather the data.** From [memory-preflight](memory-preflight.md)'s estimate,
   [real-scan-verification](../analysis/real-scan-verification.md)'s **measured peak** on the
...
2. **If there is not enough data to decide, collect it first** — do **not**
   ...
   the numbers in `docs/progress/io.md`. Re-evaluate with data in hand.
3. **Decide with a written criterion.** Proceed **only if** measured/estimated
   ...
   it, and the date in `docs/progress/io.md`.
...
- STEP 0 evaluation recorded in `docs/progress/io.md` with the numbers and
  go/no-go.
...
- [Memory preflight & in-place transform](memory-preflight.md) — provides the peak
  ...
  first.
- [Real-scan verification](../analysis/real-scan-verification.md) — supplies the **measured
