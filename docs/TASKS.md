...
- `output/sdr-preset-followups` (post-MVP; no downstream blockers): `output/presets`
  ...
  tagging it), and confirming `RunProfile::SdrTiff` against measured peak on two frame
  sizes (it currently *inherits* `HdrCodedTiff`'s arithmetic on a structural argument)
